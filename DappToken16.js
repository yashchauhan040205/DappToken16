...
contract('DappToken16', function(accounts) {
  var tokenInstance;
  ...
  it('initializes the contract with the correct values', function() {
    return DappToken16.deployed().then(function(instance) {
      ...
      assert.equal(name, 'DappToken16', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, '', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'DappToken16 v1.0', 'has the correct standard');
    });
  });
  ...
  it('allocates total supply upon deployment', function() {
    return DappToken16.deployed().then(function(instance) {
      ...
      assert.equal(adminBalance.toNumber(), 1000000, 'allocate the initial supply to admin');
    });
  });
